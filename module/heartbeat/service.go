...
func NewHeartbeatService(workspace string, intervalMinutes int, enabled bool) *HeartbeatService {
	...
	// Ensure logs directory exists at initialization (one-time operation)
...
// isHeartbeatFileEmpty checks if the heartbeat file contains only comments or blank lines.
// Returns true if all lines are empty (after trimming) or start with '#'.
func (hs *HeartbeatService) isHeartbeatFileEmpty(data []byte) bool {
	scanner := bufio.NewScanner(strings.NewReader(string(data)))
	for scanner.Scan() {
		line := scanner.Text()
		// TrimSpace handles all whitespace: space, \t, \r, \n, etc.
...
func (hs *HeartbeatService) buildPrompt() string {
	...
	// Check if file contains only comments or blank lines
	if hs.isHeartbeatFileEmpty(data) {
		logger.InfoC("heartbeat", "HEARTBEAT.md is empty (only comments/blank lines) - skipping LLM")
		return ""
	}
...
func (hs *HeartbeatService) log(level, format string, args ...any) {
	logFile := filepath.Join(hs.workspace, "logs", "heartbeat.log")
