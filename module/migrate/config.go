...
func ConvertConfig(data map[string]interface{}) (*config.Config, []string, error) {
	...
	if agents, ok := getMap(data, "agents"); ok {
		if defaults, ok := getMap(agents, "defaults"); ok {
			if v, ok := getString(defaults, "llm"); ok {
				cfg.Agents.Defaults.LLM = v
			} else if v, ok := getString(defaults, "model"); ok {
				// Old format - model without provider prefix
				// Try to infer provider from model name
				if provider := inferProviderFromModel(v); provider != "" {
					cfg.Agents.Defaults.LLM = provider + "/" + v
				} else {
					cfg.Agents.Defaults.LLM = "zhipu/" + v // Default to zhipu provider
				}
			}
	...
	if providers, ok := getMap(data, "providers"); ok {
		for name, val := range providers {
			...
			if !supportedProviders[name] {
				if apiKey != "" || apiBase != "" {
					warnings = append(warnings, fmt.Sprintf("Provider '%s' not supported in NemesisBot, skipping", name))
				}
				continue
			}

			// Convert provider to ModelConfig
			var modelIdentifier string
			var modelName string

			switch name {
			case "anthropic":
				modelIdentifier = "anthropic/claude-sonnet-4-20250514"
				modelName = "claude-sonnet-4"
			case "openai":
				modelIdentifier = "openai/gpt-4o"
				modelName = "gpt-4o"
			case "openrouter":
				modelIdentifier = "openrouter/gpt-4"
				modelName = "openrouter-model"
			case "groq":
				modelIdentifier = "groq/llama-3.3-70b-versatile"
				modelName = "groq-model"
			case "zhipu":
				modelIdentifier = "zhipu/glm-4.7-flash"
				modelName = "glm-4.7"
			case "vllm":
				modelIdentifier = "vllm/local"
				modelName = "vllm-local"
			case "gemini":
				modelIdentifier = "gemini/gemini-2.0-flash-exp"
	...
	if channels, ok := getMap(data, "channels"); ok {
		for name, val := range channels {
			...
			if !supportedChannels[name] {
				warnings = append(warnings, fmt.Sprintf("Channel '%s' not supported in NemesisBot, skipping", name))
				continue
			}
...
func MergeConfig(existing, incoming *config.Config) *config.Config {
	// Merge ModelList - add models from incoming that don't exist in existing
	for _, incomingModel := range incoming.ModelList {
		found := false
		for _, existingModel := range existing.ModelList {
			if existingModel.ModelName == incomingModel.ModelName {
				found = true
				break
			}
		}
		if !found {
			existing.ModelList = append(existing.ModelList, incomingModel)
		}
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".nemesisbot", 1)
	return path
}
