...
func Run(opts Options) (*Result, error) {
	...
	nemesisbotHome, err := resolveNemesisBotHome(opts.NemesisBotHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, nemesisbotHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to NemesisBot")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, nemesisbotHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, nemesisbotHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(nemesisbotHome, "config.json"),
				Description: "convert OpenClaw config to NemesisBot format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(nemesisbotHome)
...
func Execute(actions []Action, openclawHome, nemesisbotHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, nemesisbotHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, nemesisbotHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing NemesisBot config: %w", err)
		}
