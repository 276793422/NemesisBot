...
// Config holds configuration for the desktop UI
type Config struct {
	Enabled bool
	Width   int
	Height  int
...
// This is the new method that allows controlling the bot service from the UI
...
func bindFunctionsWithCallbacks(w webview.WebView, agentLoop *agent.AgentLoop, msgBus *bus.MessageBus, channelManager *channels.Manager) {
	...
	logger.InfoC("desktop", "Go functions bound to JavaScript")
}

// bindFunctionsWithServiceManager binds Go functions to JavaScript with ServiceManager
// This provides full control over the bot service from the UI
func bindFunctionsWithServiceManager(w webview.WebView, svcMgr *services.ServiceManager) {
	...
	logger.InfoC("desktop", "Go functions bound to JavaScript (with ServiceManager)")
}

// loadStaticFiles loads and prepares the HTML content
...
// calculateOptimalWindowSize calculates the best window size
// Default: 1280x800, but scales down if desktop is smaller
func calculateOptimalWindowSize() (width, height int) {
	...
	for i := 0; i < 20; i++ { // Max 20 iterations to prevent infinite loop
	...
	// Use minimum size if scaling failed
...
func getScreenSize() (width, height int) {
	...
	case "darwin":
		// On macOS, use system_profiler or assume common resolution
		_, err := exec.Command("system_profiler", "SPDisplaysDataType").CombinedOutput()
		if err == nil {
	...
	default: // Linux, BSD, etc.
		// On Linux, try xrandr or xdpyinfo
		// For now, assume common Full HD resolution
		return 1920, 1080
	}
}
