...
func SecurityHelp() {
	...
	fmt.Println()
	fmt.Println("Scanner Quick Start:")
	fmt.Println("  nemesisbot security scanner enable clamav   # 启用 ClamAV 引擎")
	fmt.Println("  nemesisbot security scanner check            # 查看安装状态")
	fmt.Println("  nemesisbot security scanner install          # 自动下载安装 + 病毒库")
	fmt.Println("  # 启动 gateway 后自动加载扫描引擎，文件写入时自动扫描")
	fmt.Println()
